...
async def async_init_chat_model(**kwargs):
    """Async wrapper for init_chat_model that prevents blocking the event loop.
    
    This function wraps the synchronous init_chat_model function with asyncio.to_thread
    to move the blocking operation to a separate thread.
    
    Args:
        **kwargs: Keyword arguments to pass to init_chat_model
        
    Returns:
        The initialized chat model
    """
    return await asyncio.to_thread(init_chat_model, **kwargs)
